...
app.config["MONGO_URI"] = "mongodb+srv://temp:wZSRylQ6Pxdmr5NQ@cluster0.biu1auz.mongodb.net/eweb"
mongo = PyMongo(app)

# Check if the connection is successful using init_app method
...
# Alternatively, you can also check the connection status using the following:
if mongo.cx is not None:
    print("MongoDB connected successfully!")
else:
    print("Failed to connect to MongoDB.")
...
def getAllUsers():
    ...
    users = list(allUsers.find({}))
    users_json = dumps(users, indent=2)
...
def userRegister():
    if request.method == 'POST':
        ...
        allUsers.insert_one({
            ...
            'password': hashpw,
            'cpassword': hashCpw,
            "username": request.json['companyName'],
...
def editProduct():
    ...
    if prod:
        ...
        allProducts.update_one({'_id': ObjectId(id)}, {'$set': {
                               'productName': productName, 'productPrice': productPrice, 'productUrl': productUrl}})
